...
def test_index_codebase_path_calls_index_repository(tmp_path):
    m = fresh_mcp()
    with patch("mcp_server.index_repository", return_value="Successfully indexed 10 chunks in 1.00 seconds.") as mock_idx, \
...
def test_index_codebase_github_calls_index_repository():
    ...
    with patch("mcp_server.index_repository", return_value="Successfully indexed 5 chunks in 2.00 seconds.") as mock_idx, \
         patch("mcp_server.os.makedirs"), \
         patch("builtins.open", mock_open()):
...
def test_index_codebase_resets_store():
    m = fresh_mcp()
    m._store = MagicMock()
    with patch("mcp_server.index_repository", return_value="ok"), \
         patch("mcp_server.os.makedirs"), \
         patch("builtins.open", mock_open()):
...
def test_index_codebase_returns_error_on_exception():
    m = fresh_mcp()
    with patch("mcp_server.index_repository", side_effect=RuntimeError("boom")), \
         patch("mcp_server.os.makedirs"), \
         patch("builtins.open", mock_open()):
...
def test_ask_codebase_no_results():
    m = fresh_mcp()
    m._store = _fake_store([])
    assert m.ask_codebase("anything") == "No relevant code found to answer the question."
