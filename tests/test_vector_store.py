...
def test_add_and_search_chunks(
    mock_st, temp_db_path, mock_embedding_function, sample_chunks
):
    ...
    # Check metadata
    for res in results:
        assert res.chunk_id in [
            "src/utils.py:handle_error:10",
            "src/db.py:save_to_db:20",
            "src/math.py:add:1",
        ]
...
@patch("vector_store.embedding_functions.SentenceTransformerEmbeddingFunction")
def test_duplicate_chunks_handled(
    mock_st, temp_db_path, mock_embedding_function, sample_chunks
):
    mock_st.return_value = mock_embedding_function
    store = VectorStore(persist_path=temp_db_path)
